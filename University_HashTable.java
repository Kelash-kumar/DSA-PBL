...
public class University_HashTable {
    private static class Node {
        String key;
        Pakistani_Universities value;
        Node next;
    ...
    public University_HashTable() {
        table = new Node[64];
    }// default constructor to make table of capacity of 64

    public University_HashTable(int capacity) {
    ...
    public void put(String key, Pakistani_Universities value) {
        int index = hash(key);
        Node n = table[index];
        while (n != null) {
            if (n.key.equals(key))
    ...
    public Pakistani_Universities remove(String key) {
        int index = hash(key);
        Node head = table[index];
        if (head == null)
            return null;
